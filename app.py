...
def ending():
    ...
    for idol in json_data['back_up']:
        if idol_total_point == 0:
            p = 0
        else:
            p = (idol_point[cnt] / idol_total_point) * 100
        Point.idol_dic[idol] = create_value(p, config.idol_json[idol]['name'])
        Point.idol_ins_dic[idol + '_i'] = create_ins(p, config.idol_json[idol]['name'])
